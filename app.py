...
.chat-pill:hover { border-color: #00D4FF; color: #00D4FF; }
.chat-input-row {
    display: flex;
    gap: 8px;
    padding: 12px 14px;
...
<div class="chat-bubble" id="chatBubble">
    <a href="https://wa.me/919811699944" target="_blank" class="whatsapp-btn" title="Chat with founder on WhatsApp">💬</a>
    <button class="chat-toggle" onclick="toggleChat()" title="Ask NeuroTrade AI">🧠</button>
    <div class="chat-window" id="chatWindow" style="display:none; flex-direction:column;">
        <div class="chat-header" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <button onclick="resetChat()" style="background:none; border:1px solid #334155; border-radius:6px; color:#475569; font-size:10px; padding:4px 8px; cursor:pointer;">Reset</button>
        </div>
        <div class="chat-messages" id="chatMessages">
            <div class="chat-msg-bot">Hey! I am your NeuroTrade guide. I can teach you everything on this platform in plain English. No jargon. No complexity. Just clear explanations that help you understand what the market is telling you.</div>
            <div class="chat-msg-bot" style="margin-top:4px;">Pick a topic to start learning:</div>
            <div style="display:flex; flex-wrap:wrap; gap:8px; margin-top:8px;">
                <span class="chat-pill" onclick="askQuestion('Explain the Combined Intelligence Score and what it means for my trading decisions')">🧠 Intelligence Score</span>
                <span class="chat-pill" onclick="askQuestion('Explain the Fear and Greed Index and how it influences crypto markets')">😨 Fear & Greed</span>
                <span class="chat-pill" onclick="askQuestion('What is on-chain data and why does it matter for crypto analysis')">⛓ On-Chain Data</span>
                <span class="chat-pill" onclick="askQuestion('Explain whale activity and how large transactions influence crypto prices')">🐋 Whale Activity</span>
                <span class="chat-pill" onclick="askQuestion('What is macro intelligence and how do interest rates and dollar strength affect crypto')">🌍 Macro Intelligence</span>
                <span class="chat-pill" onclick="askQuestion('Explain price momentum and 7 day trend signals and what they tell us')">📈 Price & Momentum</span>
                <span class="chat-pill" onclick="askQuestion('What is the Signal Accuracy Tracker and how does it measure performance')">🎯 Signal Accuracy</span>
                <span class="chat-pill" onclick="askQuestion('Explain news sentiment analysis and how news affects crypto prices')">📰 News Sentiment</span>
                <span class="chat-pill" onclick="askQuestion('What is the Signal Intelligence section and what do the 30 day stats mean')">📊 Signal Intelligence</span>
                <span class="chat-pill" onclick="askQuestion('Explain Ethereum gas prices and what they tell us about network activity')">⟠ ETH Gas Prices</span>
                <span class="chat-pill" onclick="askQuestion('What does trending status on CoinGecko mean and why does it matter')">🔥 Trending Status</span>
                <span class="chat-pill" onclick="askQuestion('How do I use NeuroTrade to make better informed crypto decisions')">🚀 How to use this</span>
            </div>
        </div>
        <div class="chat-input-row">
            <input class="chat-input" id="chatInput" placeholder="Ask anything about crypto or this platform..." onkeypress="if(event.key==='Enter') sendMessage()" />
...
<script>
var chatHistory = [];
var systemPrompt = `You are NeuroTrade AI, a friendly, patient and knowledgeable crypto intelligence guide built into the NeuroTrade platform. Your personality is a mix of a patient teacher and a smart friend who understands markets deeply.

Your job is to educate non-technical users about everything on the NeuroTrade dashboard. Here is everything the platform covers:

1. COMBINED INTELLIGENCE SCORE (0-100): A single score that synthesises all market signals for each asset. Scores 0-30 mean Strong Bear, 31-45 Weak Bear, 46-55 Neutral, 56-70 Weak Bull, 71-100 Strong Bull. Currently covers BTC, ETH, SOL, XRP, ADA, AVAX, DOGE, DOT and LINK. The score is built from four signals: Fear and Greed Index at 30% weight, 24 hour momentum at 25%, 7 day trend at 25%, and on-chain transaction volume at 20%.

2. FEAR AND GREED INDEX (0-100): Measures overall crypto market sentiment. 0-25 is Extreme Fear which historically is a buying opportunity. 26-45 is Fear. 46-55 is Neutral. 56-75 is Greed. 76-100 is Extreme Greed which historically precedes corrections. The platform shows today's reading, a 7 day history bar chart, and a 30 day trend line.

3. MARKET PRICES: Live prices for all 9 assets with 24 hour percentage change and market cap. Updates every 5 minutes.

4. 7-DAY PRICE HISTORY: A chart showing how the selected asset has moved over the past week with percentage change.

5. ON-CHAIN DATA for Bitcoin: Hash rate in EH/s measuring mining power and network security, Mining difficulty, Daily transaction count showing network usage, Average block time, Total BTC in circulation, Trade volume in BTC, Network health rated High Medium or Low based on transaction count, and a reference price from Blockchain.com.

6. ETHEREUM NETWORK METRICS: ETH price, 24 hour change, market cap, volume, 7 day trend, and gas prices in Gwei at Safe Standard and Fast speeds. Gas prices show how busy the Ethereum network is.

7. SOLANA METRICS: SOL price, 24 hour change, market cap and volume.

8. WHALE ACTIVITY: Shows unconfirmed Bitcoin transactions over 10 BTC in the mempool. Large transactions from wealthy wallets called whales can signal major market moves. Shows transaction hash, BTC amount, inputs and outputs, and size rating of Standard Medium or Large.

9. LATEST CRYPTO NEWS: Live news from thousands of sources with automatic sentiment labels of Bullish Neutral or Bearish based on headline keywords.

10. SIGNAL INTELLIGENCE: 30 day analysis including number of Extreme Fear days, 30 day average sentiment score, whether sentiment trend is improving or deteriorating, and consecutive days in current zone.

11. MACRO INTELLIGENCE: Four key economic indicators from the US Federal Reserve. Fed Funds Rate showing current interest rate target, US Dollar Index showing dollar strength where higher means headwinds for crypto, 10 year inflation expectations, and Unemployment rate. High rates and strong dollar are historically bearish for crypto. Low rates and weak dollar are historically bullish.

12. PRICE VS SENTIMENT HISTORY: A dual chart showing BTC price over 12 months alongside the 30 day Fear and Greed history to show how sentiment correlates with price.

13. SIGNAL ACCURACY TRACKER: Tracks how often the Fear and Greed contrarian signals have been correct. Shows Fear Reversal Signals where extreme fear preceded a price rise, and Greed Reversal Signals where extreme greed preceded a correction. Currently showing around 61 to 65 percent win rate.

14. TRENDING STATUS: Shows whether the selected asset is in the top 7 most searched coins on CoinGecko right now. Trending status can precede price movements as retail attention increases.

COMMUNICATION RULES:
- Always start by checking if the user understood your previous explanation before adding more
- If they seem confused ask them which part was unclear
- Use simple everyday analogies to explain complex concepts
- Never use jargon without immediately explaining it
- Keep each response under 150 words
- Always end with a question asking if that made sense or offering to go deeper
...
function resetChat() {
    ...
    messages.innerHTML = '<div class="chat-msg-bot">Hey! I am your NeuroTrade guide. I can teach you everything on this platform in plain English. Pick a topic below or ask me anything.</div><div style="display:flex; flex-wrap:wrap; gap:8px; margin-top:8px;"><span class="chat-pill" onclick="askQuestion(\\'Explain the Combined Intelligence Score and what it means for my trading decisions\\')">🧠 Intelligence Score</span><span class="chat-pill" onclick="askQuestion(\\'Explain the Fear and Greed Index and how it influences crypto markets\\')">😨 Fear & Greed</span><span class="chat-pill" onclick="askQuestion(\\'What is on-chain data and why does it matter for crypto analysis\\')">⛓ On-Chain Data</span><span class="chat-pill" onclick="askQuestion(\\'Explain whale activity and how large transactions influence crypto prices\\')">🐋 Whale Activity</span><span class="chat-pill" onclick="askQuestion(\\'What is macro intelligence and how do interest rates and dollar strength affect crypto\\')">🌍 Macro</span><span class="chat-pill" onclick="askQuestion(\\'Explain price momentum and 7 day trend signals and what they tell us\\')">📈 Momentum</span><span class="chat-pill" onclick="askQuestion(\\'What is the Signal Accuracy Tracker and how does it measure performance\\')">🎯 Signal Accuracy</span><span class="chat-pill" onclick="askQuestion(\\'Explain news sentiment analysis and how news affects crypto prices\\')">📰 News Sentiment</span></div>';
}
...
async function sendMessage() {
    ...
    messages.innerHTML += '<div class="chat-msg-bot" id="' + typingId + '">Thinking...</div>';
    ...
    try {
        ...
        var reply = data.content && data.content[0] ? data.content[0].text : 'Sorry I could not get a response. Try again.';
        chatHistory.push({ role: 'assistant', content: reply });
        document.getElementById(typingId).outerHTML = '<div class="chat-msg-bot">' + reply.replace(/\\n/g, '<br>') + '</div>';
    } catch(e) {
        document.getElementById(typingId).outerHTML = '<div class="chat-msg-bot">Sorry, connection issue. Try again in a moment.</div>';
    }
    messages.scrollTop = messages.scrollHeight;
}
</script>
